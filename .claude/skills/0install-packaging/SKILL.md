...
**Per-OS split feeds need a `<feed-for>` too.** When a package is split into per-OS templates/master feeds (`name-linux.xml`, `name-windows.xml`, `name-macos.xml`) aggregated by a top-level `name.xml` with `<feed arch="…-*" src="…"/>` entries (see [`gui/drawio.xml`](../../../gui/drawio.xml) + [`gui/drawio-linux.xml`](../../../gui/drawio-linux.xml)/[`-windows`](../../../gui/drawio-windows.xml)/[`-macos`](../../../gui/drawio-macos.xml), or [`gui/blender.xml`](../../../gui/blender.xml)), **each per-OS seed master feed also carries its own `<feed-for interface="https://apps.0install.net/CATEGORY/name.xml"/>`** pointing back at the top-level aggregate — placed after `<homepage>`, before the `<group>`s. This is in addition to (not instead of) the per-OS *template's* own `<feed-for>`, which points at that template's own per-OS feed (`name-linux.xml`) for 0repo merging — the two `<feed-for>` targets are different and both are needed. Without the master's `<feed-for>` back to the aggregate, tools that discover the per-OS feed directly can't trace it back to the umbrella interface.
