...
builds = dict(re.findall(r'openjdk-jdk[0-9]+u/tree/release/jdk-([0-9]+(?:\.[0-9]+)+)_([0-9]+)', data))

versions = sorted(set(re.findall(r'microsoft-jdk-([0-9]+(?:\.[0-9]+)+)-linux-x64\.tar\.gz', data)), key=lambda v: list(map(int, v.split('.'))))
