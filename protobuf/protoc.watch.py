...
def convert(release):
    ...
    protoc_asset = next((a['name'] for a in release['assets'] if a['name'].startswith('protoc-')), None)
    if protoc_asset:
        m = re.match(r'protoc-(.+?)-(linux|osx|win)', protoc_asset)
        archive_version = m.group(1) if m else version.replace('rc', 'rc-')
    else:
