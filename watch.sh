...
# Run watch scripts
#FILES=$(ls */*.watch.py | grep -v gitextensions | grep -v go-windows | grep -v kotlin | grep -v libreoffice | grep -v node | grep -v vagrant | grep -v vlc) # Exclude feeds currently not supported by 0template on Linux
