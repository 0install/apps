...
if [[ -v CI ]]; then
    ...
    FILES="golang/go-linux.watch.py golang/go-darwin.watch.py security/nmap.watch.py"
else
...
for FILE in $FILES; do
    echo "Running $FILE"
    0install run http://0install.de/feeds/0watch.xml --output ../incoming $FILE || true
done
